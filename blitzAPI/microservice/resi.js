...
module.exports = {
    ...
    updateStatus: function(connection, username, password, order_id, status) {
        return new Promise(function(resolve, reject) {
            ...
            connection.query('SELECT `id`,`password` FROM auth_user WHERE `username`=?;', [username], function (err, result) {
                ...
                transport_id = result[0]['id']
                if(authentication){
                    connection.query('UPDATE core_order SET core_order.`satus` = concat(core_order.`satus`, ?), core_order.being_delivered = 1 WHERE id = ? ; ',[status, order_id],
                        function (err, result) {
                            // console.log(result)
                            if (err) {
                                // console.log(err)
                            ...
                            return resolve('Successfully Update Delivery. On '+ new Date())
    ...
    finishDelivery: function(connection, username, password, order_id, status) {
        return new Promise(function(resolve, reject) {
            ...
            connection.query('SELECT `id`,`password` FROM auth_user WHERE `username`=?;', [username], function (err, result) {
                ...
                transport_id = result[0]['id']
                if(authentication){
                    connection.query('UPDATE core_order SET core_order.`satus` = concat(core_order.`satus`, ?), core_order.received = 1 WHERE id = ? ; ',[status, order_id],
                        function (err, result) {
                            // console.log(result)
                            if (err) {
                                // console.log(err)
                            ...
                            return resolve('Successfully Finish Delivery. On '+ new Date())
